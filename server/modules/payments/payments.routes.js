...
var { validate, paymentSchema } = require('../../shared/validation');
 ...
 * POST / — Create payment
 */
router.post('/', validate(paymentSchema), function(req, res) {
    try {
 ...
 * PUT /:id
 */
router.put('/:id', validate(paymentSchema), function(req, res) {
    try {
        var reqFy = req.headers['x-financial-year'];
        if (!isDateInActiveFY(req.body.date, reqFy)) {
            return res.status(400).json({ error: 'Transaction date cannot be outside the active financial year' });
        ...
        if (err.message === 'Payment not found') return res.status(404).json({ error: 'Payment not found' });
